...
public class Transactions extends ConsulTestBase {
  ...
  public void serviceSet(TestContext tc) {
    ...
    TxnRequest request = new TxnRequest()
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions1).setType(TxnServiceVerb.SET))
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions2).setType(TxnServiceVerb.SET));
    ctx.writeClient().transaction(request, tc.asyncAssertSuccess(response -> {
      tc.assertEquals(0, response.getErrorsSize());
      tc.assertEquals(2, response.getResultsSize());
      ...
      getRegistrations(tc, SERVICE_NAME, registrations -> {
        ...
        ctx.writeClient().deregisterService(serviceOptions1.getId(), tc.asyncAssertSuccess());
        ctx.writeClient().deregisterService(serviceOptions2.getId(), tc.asyncAssertSuccess());
      });
    }));
  }
  ...
  public void serviceGet(TestContext tc) {
    ...
    TxnRequest getRequest = new TxnRequest()
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions1).setType(TxnServiceVerb.GET))
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions2).setType(TxnServiceVerb.GET));
    ctx.writeClient().transaction(getRequest, tc.asyncAssertSuccess(resp1 -> {
      ...
      registerServiceAndGet(tc, serviceOptions1, getRequest, resp2 -> {
        ...
        registerServiceAndGet(tc, serviceOptions2, getRequest, resp3 -> {
              ...
              checkService(tc, services, serviceOptions1);
              checkService(tc, services, serviceOptions2);
              ctx.writeClient().deregisterCatalogService(ctx.nodeName(), serviceOptions1.getId(), tc.asyncAssertSuccess());
              ctx.writeClient().deregisterCatalogService(ctx.nodeName(), serviceOptions2.getId(), tc.asyncAssertSuccess());
        });
      });
    }));
  }
  ...
  public void serviceDelete(TestContext tc) {
    ...
    TxnRequest deleteRequest = new TxnRequest()
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions1).setType(TxnServiceVerb.DELETE))
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions2).setType(TxnServiceVerb.DELETE));
    ctx.writeClient().transaction(deleteRequest, tc.asyncAssertSuccess(resp1 -> {
      tc.assertEquals(0, resp1.getErrorsSize());
      tc.assertEquals(0, resp1.getResultsSize());
      ctx.writeClient().registerService(serviceOptions1, tc.asyncAssertSuccess(resp2 -> {
        getRegistrations(tc, SERVICE_NAME, registrations1 -> {
          checkService(tc, registrations1, serviceOptions1);
          ctx.writeClient().transaction(deleteRequest, tc.asyncAssertSuccess(resp3 -> {
            tc.assertEquals(0, resp1.getErrorsSize());
            tc.assertEquals(0, resp1.getResultsSize());
  ...
  public void serviceCas(TestContext tc) {
    ...
    ctx.writeClient().registerService(serviceOptions1, tc.asyncAssertSuccess(resp1 -> {
      ctx.writeClient().registerService(serviceOptions2, tc.asyncAssertSuccess(resp2 -> {
        getRegistrations(tc, SERVICE_NAME, registrations1 -> {
          ...
          TxnRequest requestWithStaleIndex = new TxnRequest()
            .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setType(TxnServiceVerb.CAS).setServiceOptions(
                serviceOptions1.setMeta(meta).setModifyIndex(idx1 - 1)))
            .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setType(TxnServiceVerb.CAS).setServiceOptions(
                serviceOptions2.setMeta(meta).setModifyIndex(idx2)));
          ctx.writeClient().transaction(requestWithStaleIndex, tc.asyncAssertSuccess(resp3 -> {
            ...
            TxnRequest request = new TxnRequest()
              ...
              .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setType(TxnServiceVerb.CAS).setServiceOptions(
                serviceOptions2.setMeta(meta).setModifyIndex(idx2)));
            ctx.writeClient().transaction(request, tc.asyncAssertSuccess(resp4 -> {
      ...
      ctx.writeClient().registerService(serviceOptions2, tc.asyncAssertSuccess(resp2 -> {
        getRegistrations(tc, SERVICE_NAME, registrations1 -> {
          ...
          TxnRequest multiOpRequest = new TxnRequest()
            .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions1).setType(TxnServiceVerb.DELETE))
            .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions2).setType(TxnServiceVerb.SET))
            .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions3).setType(TxnServiceVerb.SET));
          ctx.writeClient().transaction(multiOpRequest, tc.asyncAssertSuccess(resp3 -> {
            tc.assertEquals(0, resp3.getErrorsSize());
            tc.assertEquals(2, resp3.getResultsSize());
            getRegistrations(tc, SERVICE_NAME, registrations2 -> {
              ...
              ctx.writeClient().deregisterCatalogService(ctx.nodeName(), serviceOptions2.getId(), tc.asyncAssertSuccess());
              ctx.writeClient().deregisterCatalogService(ctx.nodeName(), serviceOptions3.getId(), tc.asyncAssertSuccess());
            });
          }));
        });
      }));
  ...
  public void serviceMultiOperationError(TestContext tc) {
    ServiceOptions serviceOptions1 = createServiceOptions("id1", SERVICE_NAME, "10.10.10.10", 8080);
    ServiceOptions serviceOptions2 = createServiceOptions("id2", SERVICE_NAME, "10.10.10.10", 8081);
    ServiceOptions serviceOptions3 = createServiceOptions("id3", "", "10.10.10.10", 8082);
    ctx.writeClient().registerService(serviceOptions1, tc.asyncAssertSuccess(resp1 -> {
      ctx.writeClient().registerService(serviceOptions2, tc.asyncAssertSuccess(resp2 -> {
        getRegistrations(tc, SERVICE_NAME, registrations1 -> {
          checkService(tc, registrations1, serviceOptions1);
  ...
  public void kvAndServiceSet(TestContext tc) {
    ...
    TxnRequest request = new TxnRequest()
      .addOperation(new TxnKVOperation().setKey("foo/bar/t1").setValue("val1").setType(TxnKVVerb.SET))
      .addOperation(new TxnKVOperation().setKey("foo/bar/t2").setValue("val2").setType(TxnKVVerb.SET))
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions).setType(TxnServiceVerb.SET));
    ctx.writeClient().transaction(request, tc.asyncAssertSuccess(response -> {
      tc.assertEquals(0, response.getErrorsSize());
      ...
      getEntries(tc, "foo/bar/t", entries -> {
        ...
        getRegistrations(tc, SERVICE_NAME, registrations2 -> {
          ...
          ctx.writeClient().deregisterCatalogService(ctx.nodeName(), serviceOptions.getId(), tc.asyncAssertSuccess());
        });
      });
    }));
  }
  ...
  public void kvAndServiceError(TestContext tc) {
    ...
    TxnRequest request = new TxnRequest()
      .addOperation(new TxnKVOperation().setKey("foo/bar/t1").setValue("val1").setType(TxnKVVerb.SET))
      .addOperation(new TxnKVOperation().setKey("foo/bar/t2").setValue("val2").setType(TxnKVVerb.SET))
      .addOperation(new TxnServiceOperation().setNode(ctx.nodeName()).setServiceOptions(serviceOptions).setType(TxnServiceVerb.GET));
    ctx.writeClient().transaction(request, tc.asyncAssertSuccess(response -> {
      ...
      ctx.readClient().getValues("foo/bar/t", tc.asyncAssertSuccess(entries -> {
  ...
  public void testJsonToTxnRequest(TestContext tc) {
    ...
    tc.assertEquals(ctx.nodeName(), deserializedTxnServiceOperation.getNode());
  ...
  private void getRegistrations(TestContext tc, String serviceName, Handler<List<Service>> resultHandler) {
    ctx.readClient().catalogServiceNodes(serviceName, tc.asyncAssertSuccess(list -> {
  ...
  private void registerServiceAndGet(TestContext tc, ServiceOptions serviceOptions, TxnRequest txnRequest, Handler<TxnResponse> resultsHandler) {
    ctx.writeClient().registerService(serviceOptions, tc.asyncAssertSuccess(resp1 -> {
      ctx.writeClient().transaction(txnRequest, tc.asyncAssertSuccess(resp2 -> {
